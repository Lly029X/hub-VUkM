...
client = openai.OpenAI(
    api_key="sk-f0ab3fca5abuabuabub5a60974549b3", # https://bailian.console.aliyun.com/?tab=model#/api-key
...
# 测试翻译智能体
result = ExtractionAgent(model_name = "qwen-plus").call('帮我将good！翻译为中文', TranslationRequest)
print("翻译请求解析结果:", result)
...
def translate_text(source_language, target_language, text):
    messages = [
        {
            "role": "user",
            "content": f"请将以下{source_language}文本翻译为{target_language}：{text}"
        }
    ]
    response = client.chat.completions.create(
        model="qwen-plus",
...
# 执行翻译
if result:
    translation_result = translate_text(result.source_language, result.target_language, result.text)
