...
class GRUClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
        ...
        self.gru = nn.GRU(embedding_dim, hidden_dim, batch_first=True)  # GRU循环层
    ...
    def forward(self, x):
        ...
        # GRU只返回hidden_state，没有cell_state
        gru_out, hidden_state = self.gru(embedded)

        # batch size * output_dim
        # GRU的hidden_state形状为 (num_layers * num_directions, batch_size, hidden_dim)
        # 需要选择最后一个时间步的隐藏状态
